      ...
      <Col>
        <div className="event-head">
          <div className="event-headline">
            <h2><Moment format="DD.MM.YYYY - HH.mm">{event.date}</Moment></h2>
          </div>
          <div className="event-headline">
            <h2>{event.name}</h2>
          </div>
        ...
        <Col className="event-body">
          {event.entrance_vvk && `Entrance VVK: ${event.entrance_vvk} ${event.entrance_ak ? ', ' : ''} `}
          {event.entrance_ak && `Entrance AK: ${event.entrance_ak}`}
          <br />
          <br />
          <p>{event.event_description}</p>
          <br />
          {event.artist_details.map((artist_details, index, array) => (
            <div key={index}>
