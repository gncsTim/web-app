...
class HeadlinerForm extends React.Component {
  ...
  handleAddition(key) {
    return (tag) => {
      if (!key) {
        return this.props.updateState({ headlinerGenre: this.props.headlinerGenre.concat([tag]) })
      }
    }
  }
                ...
                <Row>
                  {headlinerLinks.map((item, index) => (
                    <Col xs={12} md={6} key={index}>
                      <Form.Control type="text" onChange={this.handleChangeHeadlinerLinks(index)} />
                    </Col>
                  ))}
                </Row>
              </Col>
